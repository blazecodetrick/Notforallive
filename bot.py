...
# ================= Configuration =================
# All secrets are now read from Railway environment variables.
# Set these in Railway -> your service -> Variables tab (never hardcode them here).
def _require_env(name):
    val = os.environ.get(name)
    if not val:
        raise RuntimeError(f"Missing required environment variable: {name}. Set it in Railway's Variables tab.")
    return val
